...
# Use the cuda-12.8-toolchain image for building TensorFlow I/O (see common/linux_arm64/ubuntu20.04/cuda-12.8-toolchain)
...
# Clone TensorFlow I/O and install the Ubuntu 20.04 wheel for TensorFlow 2.19.1.4 (after resolving deps with regular 2.19.1)
RUN git clone --depth 1 https://github.com/andersensam/tensorflow-io && \
    ...
    uv pip install --no-deps --no-index https://github.com/andersensam/tensorflow/releases/download/2.19.1.4/tensorflow-2.19.1.4-cp312-cp312-manylinux_2_31_aarch64.whl && \
    uv cache clean
...
COPY tfio_0.37.3_ubuntu20.04_arm64.brc .bazelrc
...
RUN rm -rf build && \
    ...
    python3 setup.py --data build bdist_wheel --plat-name manylinux_2_31_aarch64 && \
    rm -rf build && \
    mkdir build && \
    cp -r -L bazel-bin/tensorflow_io_gcs_filesystem  build/tensorflow_io_gcs_filesystem && \
    python3 setup.py --data build bdist_wheel --project tensorflow-io-gcs-filesystem --plat-name manylinux_2_31_aarch64 && \
