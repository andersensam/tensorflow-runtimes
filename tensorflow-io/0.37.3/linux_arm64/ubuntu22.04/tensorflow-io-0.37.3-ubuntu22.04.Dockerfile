...
ARG BASE_IMAGE=cuda-12.8-toolchain:ubuntu22.04-arm64

# Use the cuda-12.8-toolchain image for building TensorFlow I/O (see common/linux_arm64/ubuntu22.04/cuda-12.8-toolchain)
...
RUN git clone --depth 1 https://github.com/andersensam/tensorflow-io && \
    ...
    uv pip install --no-deps --no-index https://storage.googleapis.com/axlearn-wheels/tensorflow/tensorflow-2.19.1.3-cp312-cp312-manylinux_2_35_aarch64.whl && \
    uv cache clean
...
COPY tfio_0.37.3_ubuntu22.04_arm64.brc .bazelrc
...
RUN rm -rf build && \
    ...
    python3 setup.py --data build bdist_wheel --plat-name manylinux_2_35_aarch64 && \
    rm -rf build && \
    mkdir build && \
    cp -r -L bazel-bin/tensorflow_io_gcs_filesystem  build/tensorflow_io_gcs_filesystem && \
    python3 setup.py --data build bdist_wheel --project tensorflow-io-gcs-filesystem --plat-name manylinux_2_35_aarch64 && \
