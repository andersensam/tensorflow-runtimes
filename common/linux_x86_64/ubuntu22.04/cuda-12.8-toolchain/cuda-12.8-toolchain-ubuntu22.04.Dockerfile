...
RUN apt-get update && \
    ...
    curl -o LLVM-20.1.7.tar.xz https://github.com/llvm/llvm-project/releases/download/llvmorg-20.1.7/LLVM-20.1.7-Linux-X64.tar.xz && \
...
RUN apt-get update && apt-get install -y curl && \
    ...
    apt-get update && apt-get install -y cuda-libraries-dev-12-8 libcudnn9-dev-cuda-12 libnccl-dev ibverbs-utils \
         ...
         cuda-cupti-12-8 cuda-cupti-dev-12-8 nano && \
    apt clean -y && \
    rm -rf /var/lib/apt/lists/*
