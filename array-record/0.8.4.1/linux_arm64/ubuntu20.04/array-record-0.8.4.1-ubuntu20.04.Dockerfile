...
RUN uv pip install patchelf auditwheel && \
    python3 setup.py bdist_wheel --python-tag py312 && \
    auditwheel repair --plat manylinux_2_31_aarch64 -w repaired_dist dist/*.whl && \
    uv pip install https://github.com/andersensam/tensorflow/releases/download/2.21.0.3/tensorflow-2.21.0.3-cp312-cp312-manylinux_2_31_aarch64.whl repaired_dist/*.whl && \
