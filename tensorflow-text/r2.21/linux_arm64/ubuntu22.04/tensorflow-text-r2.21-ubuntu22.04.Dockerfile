...
COPY text_r2.21_ubuntu22.04_arm64.brc .bazelrc
COPY text_r2.21_ubuntu22.04_arm64.tfc .tf_configure.bazelrc
# Use the TensorFlow cache, if it exists since it pulls many of the same deps
RUN --mount=type=cache,target=/root/.cache/bazel,id=bazel-cache-r2.21.0.1-ubuntu22.04-arm64 \
