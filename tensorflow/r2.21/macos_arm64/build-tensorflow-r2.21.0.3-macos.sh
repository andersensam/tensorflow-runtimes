...
# Clone TensorFlow
cd build && git clone --depth 1 --branch r2.21 https://github.com/andersensam/tensorflow
...
cat ../tf_r2.21.0.3_macos_arm64.brc >> tensorflow/.tf_configure.bazelrc
...
cd tensorflow && ../bazel build //tensorflow/tools/pip_package:wheel --repo_env=WHEEL_NAME=tensorflow --config=macos_arm64 \
    --verbose_failures --define=no_system_libs=true --action_env TF_SYSTEM_LIBS="" --spawn_strategy=darwin-sandbox